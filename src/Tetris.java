...
public class Tetris extends JFrame implements ActionListener, KeyListener, MouseMotionListener, MouseWheelListener {
	...
	final static Color[] foregroundColors = { Color.black, new Color(10, 212, 252), new Color(12, 15, 240),
			new Color(240, 160, 12), new Color(247, 228, 10), new Color(19, 235, 33), new Color(201, 16, 230),
			new Color(230, 28, 25), Color.white };
	final static Color[] backgroundColors = { Color.black, new Color(33, 165, 191), new Color(35, 37, 158),
			new Color(181, 111, 31), new Color(194, 180, 29), new Color(17, 173, 27), new Color(164, 20, 186),
			new Color(181, 24, 22), Color.white };
...
//		}
		...
		for (int i = 1; i <= 7; i++) {
			try {
				InputStream imageStream = loader.getResourceAsStream(i + ".png");
		...
		// Music
		try {
			InputStream song1Stream = loader.getResourceAsStream("song1.wav");
			InputStream song2Stream = loader.getResourceAsStream("song2.wav");
			InputStream song3Stream = loader.getResourceAsStream("song3.wav");
			InputStream loseSongStream = loader.getResourceAsStream("loseSong.wav");
...
//			br.close();
			...
			NetworkHandler.publishScore(new Score(newName, newScore));
			scores = NetworkHandler.getScores();
...
//			writer.close();
		...
		public void paintComponent(Graphics g) {
			...
			if (gameState == 0) {
				try {
					...
					g.drawString(TITLE, (WINDOWWIDTH / 2) - (int) ((g.getFontMetrics(determinationSans.deriveFont(100f))
							.getStringBounds(TITLE, g).getWidth() / 2)), 90);
					g.setFont(determinationSans.deriveFont(50f));
					g.drawString("A wonderful experience",
							(WINDOWWIDTH / 2) - (int) ((g.getFontMetrics(determinationSans.deriveFont(50f))
									.getStringBounds("A wonderful experience", g).getWidth() / 2)),
							150);
					g.drawString("by Dylan Speiser",
							(WINDOWWIDTH / 2) - (int) ((g.getFontMetrics(determinationSans.deriveFont(50f))
									.getStringBounds("by Dylan Speiser", g).getWidth() / 2)),
							200);
					...
					for (int i = 0; i < menuStrings.length; i++) {
						...
						g.drawString(menuStrings[i],
								(int) ((WINDOWWIDTH / 2) - ((g.getFontMetrics(determinationMono)
										.getStringBounds(menuStrings[i], g).getWidth() / 2))),
								(int) ((450 + (i * 200)) * rescale.y));
					}
				} catch (Exception e) {
				}
			} else if (gameState == 1) {
				...
				g.drawString("Please enter your name:",
						(int) ((WINDOWWIDTH / 2) - ((g.getFontMetrics(g.getFont())
								.getStringBounds("Please enter your name:", g).getWidth() / 2))),
						(int) (350 * rescale.y));
				...
				for (int i = 0; i < name.length(); i++) {
					...
					g.drawString(name.charAt(i) + "",
							(int) ((WINDOWWIDTH / 2) - 195 + (i * 50) + Math.random() * 5 * (rescale.x)),
							(int) (500 + Math.random() * 5 * rescale.y));
				}
				g.setFont(determinationSans.deriveFont(50f));
				...
				g.drawString("Press ENTER when you are happy with your name.",
						(int) ((WINDOWWIDTH / 2) - ((g.getFontMetrics(g.getFont())
								.getStringBounds("Press ENTER when you are happy with your name.", g).getWidth() / 2))),
						(int) (700 * rescale.y));
			} else if (gameState >= 2 && gameState != 99) {
				// Main Board
				Color borderColor;
				...
				g.drawString(name, (int) (((WINDOWWIDTH / 2 - 400) + (WINDOWWIDTH / 2 - 200)) / 2
						- ((g.getFontMetrics(g.getFont()).getStringBounds(name, g)).getWidth() / 2)), 250);
				g.drawString(score + "",
						(int) (((WINDOWWIDTH / 2 - 400) + (WINDOWWIDTH / 2 - 200)) / 2
								- ((g.getFontMetrics(g.getFont()).getStringBounds(score + "", g)).getWidth() / 2)),
						300);
				...
				g.setColor(borderColor.brighter().brighter());
				g.drawString("HOLD", (int) (((WINDOWWIDTH / 2 - 400) + (WINDOWWIDTH / 2 - 200)) / 2
						- ((g.getFontMetrics(g.getFont()).getStringBounds("HOLD", g)).getWidth() / 2)), 855);
				...
				g.setColor(borderColor.brighter().brighter());
				g.drawString("NEXT", (int) (((WINDOWWIDTH / 2 + 200) + (WINDOWWIDTH / 2 + 400)) / 2
						- ((g.getFontMetrics(g.getFont()).getStringBounds("NEXT", g)).getWidth() / 2)), 250);
				...
				for (Piece p : ghost.pieces) {
					g.fillRect((int) tileCorners[(int) p.index.y][(int) p.index.x].x,
							(int) tileCorners[(int) p.index.y][(int) p.index.x].y, 40, 40);
				}
				g.setColor(Color.black);
				for (Piece p : ghost.pieces) {
					g.fillRect((int) tileCorners[(int) p.index.y][(int) p.index.x].x + 3,
							(int) tileCorners[(int) p.index.y][(int) p.index.x].y + 3, 34, 34);
				}
				...
				if (flavorTimer.isRunning()) {
					...
					g.drawString(tetrisFlavorString,
							(int) (rescale.x * 720 - (g.getFontMetrics(determinationSans.deriveFont(30f))
									.getStringBounds(tetrisFlavorString, g)).getWidth()),
							140 + (flavorAnimationTime / 2));
				}
			...
			if (gameState == 3) {
				...
				g.drawString("HIGHSCORES", (int) (WINDOWWIDTH / 2
						- ((g.getFontMetrics(determinationSans.deriveFont(40f)).getStringBounds("HIGHSCORES", g))
								.getWidth() / 2)),
						240);
				...
				for (int i = 0; i < Math.min(scoreStrings.size(), 18); i++) {
					...
					g.drawString(scoreStrings.get(i), (int) (WINDOWWIDTH / 2
							- ((g.getFontMetrics(g.getFont()).getStringBounds(scoreStrings.get(0), g)).getWidth() / 2)),
							290 + (i * 40));
				}
			}
			if (gameState == 99) {
				...
				for (int i = 0; i < Math.min(scoreStrings.size(), scoresPerRow); i++) {
					if (place < 10) {
						g.drawString(place + ".  " + scoreStrings.get(i),
								(int) (WINDOWWIDTH / 2 - 500 - 60 * rescale.x
										- ((g.getFontMetrics(g.getFont()).getStringBounds(scoreStrings.get(0) + " ", g))
												.getWidth() / 2)),
								230 + (i * 40));
					} else {
						g.drawString(place + ". " + scoreStrings.get(i),
				...
				if (scoreStrings.size() > scoresPerRow) {
					for (int i = 0; i < Math.min(scoreStrings.size() - scoresPerRow, scoresPerRow); i++) {
						g.drawString(place + ". " + scoreStrings.get(scoresPerRow + i),
								(int) (WINDOWWIDTH / 2 - 60 * rescale.x
										- ((g.getFontMetrics(g.getFont()).getStringBounds(scoreStrings.get(0) + " ", g))
												.getWidth() / 2)),
								230 + (i * 40));
						place++;
					...
					if (scoreStrings.size() > 2 * scoresPerRow) {
						for (int i = 0; i < Math.min(scoreStrings.size() - (2 * scoresPerRow), scoresPerRow); i++) {
							g.drawString(place + ". " + scoreStrings.get(2 * scoresPerRow + i),
									(int) (WINDOWWIDTH / 2 + 500 - 60 * rescale.x - ((g.getFontMetrics(g.getFont())
											.getStringBounds(scoreStrings.get(0) + " ", g)).getWidth() / 2)),
									230 + (i * 40));
							place++;
						}
					}
				}
				g.drawString("Press BACKSPACE to return to the menu.",
						(int) ((WINDOWWIDTH / 2) - ((g.getFontMetrics(g.getFont())
								.getStringBounds("Press BACKSPACE to return to the menu.", g).getWidth() / 2))),
						(int) (1040 * rescale.y));
			}
			...
			if (debug) {
				...
				g.drawString("Window Dimensions: " + this.getParent().getWidth() + " x " + this.getParent().getHeight(),
						4, 284);
				...
				if (gameState == 1) {
					...
					g.drawString("FlashTimer: Running: " + flashTimer.isRunning() + " Delay: " + flashTimer.getDelay(),
							4, 184);
					...
					for (int i = 0; i < activeTetromino.pieces.size(); i++) {
						g.drawString(i + "",
								(int) tileCorners[(int) activeTetromino.pieces
										.get(i).index.y][(int) activeTetromino.pieces.get(i).index.x].x + 15,
								(int) tileCorners[(int) activeTetromino.pieces
										.get(i).index.y][(int) activeTetromino.pieces.get(i).index.x].y + 30);
					}
				}
			}
		}
	...
	public class Tetromino {
		...
		public void rotate(int direction) {
			...
			if (type == 1
					&& Math.min(Math.min(Math.min(pieces.get(0).index.y, pieces.get(1).index.y), pieces.get(2).index.y),
							pieces.get(3).index.y) >= 1) {
				int[][] rotationMatrixX = { { 2, 1, -2, -1 }, { 1, 0, -1, 0 }, { 0, -1, 0, 1 }, { -1, -2, 1, 2 } };

				int[][] rotationMatrixY = { { -1, 2, 1, -2 }, { 0, 1, 0, -1 }, { 1, 0, -1, 0 }, { 2, -1, -2, 1 } };
				...
				int[] rotationMatrixX = { 2, 1, 0, 1, 0, -1, 0, -1, -2, };

				int[] rotationMatrixY = { 0, 1, 2, -1, 0, 1, -2, -1, 0, };

				int count = 0;
				for (int y = -1; y <= 1; y++) {
					for (int x = -1; x <= 1; x++) {
						for (int p = 0; p < pieces.size(); p++) {
							if (pieces.get(p).index.x == (int) pieces.get(2).index.x + x
									&& pieces.get(p).index.y == (int) pieces.get(2).index.y + y)
